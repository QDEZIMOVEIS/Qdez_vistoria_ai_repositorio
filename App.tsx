...
const App: React.FC = () => {
  ...
  const startNewSequence = () => { setView('type_selector'); };
  ...
  const handleRunComparison = async () => {
    if (!pdfEntry || !pdfExit || !current) {
      alert("Por favor, selecione os dois arquivos PDF para análise.");
      return;
    }
    setIsBusy(true);
    try {
      const res = await performComparisonAI(pdfEntry, pdfExit, settings, manualComparisonObs);
      const updatedCurrent: Inspection = { 
        ...current, 
        comparisonResult: {
          ...
          <div className="max-w-xl mx-auto py-10 animate-in fade-in zoom-in-95 duration-300">
            <div className="bg-white p-10 rounded-[3rem] shadow-2xl border border-slate-100">
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-3 gap-2 p-1 bg-slate-50 rounded-2xl border border-slate-100">
                    {(['Conciso', 'Normal', 'Muito Detalhado'] as const).map(option => (
                      <button key={option} onClick={() => setSettings(s => ({ ...s, detailLevel: option }))} className={`py-3 text-[9px] font-black uppercase rounded-xl transition-all ${settings.detailLevel === option ? 'bg-slate-900 text-white shadow-lg' : 'text-slate-400 hover:text-slate-600'}`}>
                        {option}
                      </button>
                    ))}
                  </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-3 gap-2 p-1 bg-slate-50 rounded-2xl border border-slate-100">
                    {(['Baixa', 'Média', 'Alta'] as const).map(option => (
                      <button key={option} onClick={() => setSettings(s => ({ ...s, defaultSeverity: option }))} className={`py-3 text-[9px] font-black uppercase rounded-xl transition-all ${settings.defaultSeverity === option ? 'bg-red-600 text-white shadow-lg' : 'text-slate-400 hover:text-slate-600'}`}>
                        {option}
                      </button>
                    ))}
                  </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-3 gap-2 p-1 bg-slate-50 rounded-2xl border border-slate-100">
                    {(['Técnico', 'Formal', 'Direto'] as const).map(option => (
                      <button key={option} onClick={() => setSettings(s => ({ ...s, tone: option }))} className={`py-3 text-[9px] font-black uppercase rounded-xl transition-all ${settings.tone === option ? 'bg-slate-900 text-white shadow-lg' : 'text-slate-400 hover:text-slate-600'}`}>
                        {option}
                      </button>
                    ))}
                  </div>
              ...
              <div className="mt-10 pt-8 border-t border-slate-100">
                <button onClick={() => setView('list')} className="w-full bg-slate-900 text-white py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-xl active:scale-95 transition-all">Salvar e Voltar</button>
              </div>
            </div>
          </div>
        )}
                  ...
                  inspections.filter(i => i.status === 'draft').map(ins => (
                    <div key={ins.id} className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-200 flex justify-between items-center hover:border-red-500 transition-all cursor-pointer group" onClick={() => { setCurrent(ins); setView(ins.type === 'Comparação' ? 'comparison' : 'editor'); }}>
                      ...
                      <div className="flex items-center gap-2">
                        <button onClick={(e) => deleteInspection(ins.id, e)} className="w-10 h-10 rounded-full flex items-center justify-center hover:bg-red-50 text-slate-300 hover:text-red-600 transition-all active:scale-90" title="Excluir Vistoria">
            ...
            <div className="bg-white p-10 rounded-[3rem] shadow-2xl border border-slate-100 text-center">
              ...
              <div className="grid gap-3">
                ...
                <button onClick={() => confirmNewInspection('Comparação')} className="p-5 rounded-2xl border-2 border-slate-100 hover:border-red-500 hover:bg-red-50 transition-all text-left group">
                  <span className="block font-black text-slate-800 group-hover:text-red-600 uppercase text-xs">Comparativo Pericial</span>
                  <span className="text-[9px] font-medium text-slate-400 uppercase">Análise de divergências entre PDFs</span>
                </button>
              </div>
        ...
        {view === 'comparison' && current && (
          <div className="max-w-xl mx-auto space-y-6 pt-10 pb-40 animate-in fade-in duration-500">
             <div className="bg-white p-10 rounded-[3rem] shadow-2xl border border-slate-100">
               ...
               <p className="text-[10px] text-slate-400 font-bold uppercase mb-10 tracking-[0.2em]">Perícia Digital de Divergências</p>
               
               <div className="space-y-6">
                 {/* Upload Laudo Entrada */}
                 <div className={`p-8 rounded-[2rem] border-2 border-dashed transition-all relative ${pdfEntry ? 'bg-green-50 border-green-200' : 'bg-slate-50 border-slate-200 hover:border-red-400'}`}>
                   <label className="text-[10px] font-black uppercase text-slate-400 block mb-4">Laudo de ENTRADA (PDF)</label>
                   {pdfEntry ? (
                     <div className="flex items-center justify-between">
                       <span className="text-[10px] font-black text-green-700 uppercase">Arquivo Carregado ✓</span>
                       <button onClick={() => setPdfEntry(null)} className="text-[10px] font-black text-red-600 uppercase underline">Trocar</button>
                     </div>
                   ) : (
                     <input type="file" accept=".pdf" onChange={(e) => handleFileUpload(e, setPdfEntry)} className="text-[10px] font-bold w-full cursor-pointer file:bg-slate-900 file:text-white file:border-none file:px-4 file:py-2 file:rounded-xl" />
                   )}
                 </div>

                 {/* Upload Laudo Saída */}
                 <div className={`p-8 rounded-[2rem] border-2 border-dashed transition-all relative ${pdfExit ? 'bg-green-50 border-green-200' : 'bg-slate-50 border-slate-200 hover:border-red-400'}`}>
                   <label className="text-[10px] font-black uppercase text-slate-400 block mb-4">Laudo de SAÍDA (PDF)</label>
                   {pdfExit ? (
                     <div className="flex items-center justify-between">
                       <span className="text-[10px] font-black text-green-700 uppercase">Arquivo Carregado ✓</span>
                       <button onClick={() => setPdfExit(null)} className="text-[10px] font-black text-red-600 uppercase underline">Trocar</button>
                     </div>
                   ) : (
                     <input type="file" accept=".pdf" onChange={(e) => handleFileUpload(e, setPdfExit)} className="text-[10px] font-bold w-full cursor-pointer file:bg-slate-900 file:text-white file:border-none file:px-4 file:py-2 file:rounded-xl" />
                   )}
                 </div>

                 {/* Observações Manuais */}
                 <div className="space-y-3">
                   <label className="text-[10px] font-black uppercase text-slate-400 ml-1">Foco Pericial do Corretor</label>
                   <textarea 
                     ...
                     placeholder="Aponte danos específicos (ex: 'vidro trincado na sala'), faltas de chaves ou detalhes que a IA deve cruzar..."
               ...
               >
                 {isBusy ? "David Oliveira Periciando..." : "Iniciar Perícia Comparativa IA"}
               </button>
               
               <button onClick={() => setView('list')} className="w-full mt-6 text-[10px] font-black text-slate-400 uppercase tracking-widest hover:text-slate-600 transition-colors">Cancelar</button>
             </div>
          </div>
        )}
          ...
          <div className="space-y-6 pb-40 animate-in fade-in duration-500">
            ...
            <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100 relative overflow-hidden">
              ...
              <div className="flex justify-between items-start mb-6">
                <input className="flex-1 text-2xl font-black border-none focus:ring-0 p-0 placeholder:text-slate-200 bg-transparent uppercase tracking-tighter" placeholder="Endereço do Imóvel" value={current.address} onChange={(e) => updateCurrent({ address: e.target.value })} />
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-1"><label className="text-[9px] font-black text-slate-400 uppercase ml-1">Proprietário / Cliente</label><input className="w-full bg-slate-50 p-4 rounded-2xl text-xs font-bold border-none" value={current.clientName} onChange={(e) => updateCurrent({ clientName: e.target.value })} /></div>
                <div className="space-y-1"><label className="text-[9px] font-black text-slate-400 uppercase ml-1">Locatário / Interessado</label><input className="w-full bg-slate-50 p-4 rounded-2xl text-xs font-bold border-none" value={current.tenantName} onChange={(e) => updateCurrent({ tenantName: e.target.value })} /></div>
                <div className="space-y-1 col-span-2"><label className="text-[9px] font-black text-slate-400 uppercase ml-1">Vistoriador Responsável</label><input className="w-full bg-slate-100 p-4 rounded-2xl text-[10px] font-black border-none text-slate-500 cursor-not-allowed" readOnly value={current.inspectorName} /></div>
              </div>
            </div>
      ...
      {isBusy && (
        <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-2xl z-[100] flex items-center justify-center p-10 animate-in fade-in duration-500">
          <div className="bg-white p-16 rounded-[4rem] max-w-sm w-full text-center shadow-[0_0_120px_rgba(220,38,38,0.15)] border border-red-50">
            ...
            <p className="text-[11px] text-slate-400 font-bold uppercase leading-relaxed tracking-[0.2em]">Cruzando dados dos PDFs e pesquisando valores de reparo via Google Search...</p>
          </div>
        </div>
      )}
    </div>
